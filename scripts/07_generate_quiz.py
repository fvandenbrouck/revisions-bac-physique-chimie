...
from config import ALLOWED_THEMES, CLAUDE_MAX_TOKENS_QUIZ, CLAUDE_MODEL, CLAUDE_TEMPERATURE, COURSES_JSON, QUIZ_JSON
...
def call(client: Anthropic, theme_id: str, course: dict[str, Any], n: int, model: str, max_retries: int) -> list[dict[str, Any]]:
    last_error = None
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"Erreur quiz {theme_id} tentative {attempt}/{max_retries}: {exc}. Pause {sleep}s", file=sys.stderr)
            time.sleep(sleep)
...
def main() -> None:
    load_dotenv()
    parser = argparse.ArgumentParser(description="Génère un quiz local par thématique à partir des fiches de cours.")
    parser.add_argument("--model", default=CLAUDE_MODEL)
    parser.add_argument("--questions-per-theme", type=int, default=8)
    ...
    for theme_id in ALLOWED_THEMES:
        ...
        if not course:
            quiz[theme_id] = []
            continue
        quiz[theme_id] = call(client, theme_id, course, args.questions_per_theme, args.model, args.max_retries)
